...
[Shiba Inu](https://en.wikipedia.org/wiki/Shiba_Inu). All that remains of the manual are some indicator light diagrams, 
button wiring schematics, and [joltage](https://adventofcode.com/2025/day/3) requirements for each machine.
...
[.###.#] (0,1,2,3,4) (0,3,4) (0,1,2,4,5) (1,2) {10,11,11,5,10,5}
```
Configuring the first machine's counters requires a minimum of **`10`** button presses. One way to do this is by 
pressing `(3)` once, `(1,3)` three times, `(2,3)` three times, `(0,2)` once, and `(0,1)` twice.

Configuring the second machine's counters requires a minimum of **`12`** button presses. One way to do this is by 
pressing `(0,2,3,4)` twice, `(2,3)` five times, and `(0,1,2)` five times.
